...
public class DataPlanePort implements Runnable, Callback {
	int portNum; //Port Number of DataPlanePort
	int nNeighbor; //Number of neighbors of Source Node.
	DataOutputStream[] outs; // An array of DataOutPutStreams for each neighbor.
	Listner[] listners; //An array of listeners for each neighbor.
	ConcurrentLinkedQueue<MessageType> que; //
	HashMap<Integer, Integer> neighbor2stream; 		// mapping node_num to outStream_num


	//Constructor to initialize DataPlanePort.
	//Retrieves port number and neighbors the node that instantiated DataPlanePort.
	...
	//Creates a serverSocket that waits for input from neigbor's PortUser.
	public synchronized void initialize() {
		...
		for (int j = 0; j < nNeighbor; j++) { //This for loop is for connecting to every Neighbor's PortUser.
			int nodeNum = -1;
       		try {
